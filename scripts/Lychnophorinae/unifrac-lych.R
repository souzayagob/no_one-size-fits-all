...
lych_tree <- drop.tip(lych_tree, 
                        lych_tree$tip.label[!lych_tree$tip.label %in% colnames(lych_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(lych_matrix, lych_tree))
write.csv(file = "results/Lychnophorinae/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(lych_matrix, lych_tree, dist = "unifrac",
...
png("plots/Lychnophorinae/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Lychnophorinae/upgma/unifrac_dend.pdf", 

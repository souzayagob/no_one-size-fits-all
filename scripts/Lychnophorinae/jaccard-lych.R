...
lych_matrix <- lych_matrix[ , which(colSums(lych_matrix) > 1)]

#Removing empty sites 
lych_matrix <- lych_matrix[which(rowSums(lych_matrix) > 0), ]
...
jaccard_matrix <- as.matrix(vegdist(lych_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Lychnophorinae/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Lychnophorinae/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Lychnophorinae/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(lych_matrix, dist = "jaccard",
...
png("plots/Lychnophorinae/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Lychnophorinae/upgma/jaccard_dend.pdf", 

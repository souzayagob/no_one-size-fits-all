...
#Reading tree
lych_tree <- read.nexus("trees/Lychnophorinae/pruned_tree-lych.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(lych_matrix), 
                 "pe" = phyloendemism(lych_matrix, lych_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(lych_tree, lych_matrix))
rownames(null.output) <- rownames(lych_matrix)
...
#Calculating PE
pe_comp <- data.frame("id" = rownames(lych_matrix), 
                      "pe_comp" = phyloendemism(lych_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, lych_matrix))
rownames(null.output) <- rownames(lych_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(lych_tree, lych_matrix))
rownames(null.output) <- rownames(lych_matrix)
...
png("plots/Lychnophorinae/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Lychnophorinae/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Lychnophorinae/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Lychnophorinae/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Lychnophorinae/canape/canape_plot.png",

...
coords <- read.csv(file = "datasets/Lychnophorinae/lych_cr.csv", na.strings = c("", NA))

# Reading PD results
pd <- read.csv("results/Lychnophorinae/pd_stats.csv")
pd <- pd %>% rename("id_grid" = id)

# Reading tree
tree <- read.nexus("trees/Lychnophorinae/pruned_tree-lych.nex")
...
corrPdRed_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Lychnophorinae", subtitle = paste("r =", corrPdRed, "\np-value < 0.05, R² = 0.35"))+
  xlab("Redundancy")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text())+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Lychnophorinae/lych-corrPdRed_plot.pdf"); corrPdRed_plot; dev.off()
...
corrPdN_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Lychnophorinae", subtitle = paste("r =", corrPdN, "\np < 0.05, R² = 0.61"))+
  xlab("Number of samples")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text())+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Lychnophorinae/lych-corrPdN_plot.pdf"); corrPdN_plot; dev.off()

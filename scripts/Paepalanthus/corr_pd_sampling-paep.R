...
coords <- read.csv(file = "datasets/Paepalanthus/paep_cr.csv", na.strings = c("", NA))

# Reading PD results
pd <- read.csv("results/Paepalanthus/pd_stats.csv")
pd <- pd %>% rename("id_grid" = id)

# Reading tree
tree <- read.nexus("trees/Paepalanthus/pruned_tree-paep.nex")

...
jaccard_matrix <- as.matrix(vegdist(paep_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Paepalanthus/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Paepalanthus/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Paepalanthus/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(paep_matrix, dist = "jaccard",
...
png("plots/Paepalanthus/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Paepalanthus/upgma/jaccard_dend.pdf", 

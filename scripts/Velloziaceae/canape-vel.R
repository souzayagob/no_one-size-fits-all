...
#Calculating PE
pe <- data.frame("id" = rownames(vel_matrix), 
                 "pe" = phyloendemism(vel_matrix, vel_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(vel_tree, vel_matrix))
rownames(null.output) <- rownames(vel_matrix)
...
#Calculating PE
pe_comp <- data.frame("id" = rownames(vel_matrix), 
                      "pe_comp" = phyloendemism(vel_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, vel_matrix))
rownames(null.output) <- rownames(vel_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(vel_tree, vel_matrix))
rownames(null.output) <- rownames(vel_matrix)
...
png("plots/Velloziaceae/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Velloziaceae/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Velloziaceae/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Velloziaceae/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Velloziaceae/canape/canape_plot.png",

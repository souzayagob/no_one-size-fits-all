...
#Saving results
write.csv(file = "results/Velloziaceae/redundancy.csv", redundancy, row.names = FALSE)
...
#Loading results
redundancy <- read.csv("results/Velloziaceae/redundancy.csv")
...
redMedian_plot <- ggplot(data = redundancy, mapping = aes(x = grid_size, y = median)) + 
  ...
  geom_segment(data = redundancy[which(redundancy$grid_size == 0.6), ], aes(yend=median), xend=-1, #alpha = 0.5,
               linetype = "dashed") +
  labs(title = "Velloziaceae")+
  labs(y= "Redundancy", x = "Grid resolution")+
  theme_bw(base_size = 14)
png("plots/Velloziaceae/redundancy/redMedian_plot.png",
...
redMean_plot <- ggplot(data = redundancy, mapping = aes(x = grid_size, y = mean)) + 
  ...
  theme_bw()
png("plots/Velloziaceae/redundancy/redMean_plot.png",

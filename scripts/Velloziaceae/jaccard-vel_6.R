...
jaccard_matrix <- as.matrix(vegdist(vel_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Velloziaceae/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Velloziaceae/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Velloziaceae/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(vel_matrix, dist = "jaccard",
...
pdf("plots/Velloziaceae/upgma/jaccard_plot-6.pdf",
    height = 4, width = 4); jaccard_plot; dev.off()
pdf("plots/Velloziaceae/upgma/jaccard_dend-6.pdf", 

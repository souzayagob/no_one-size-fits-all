...
set.seed(7)
pd_stats <- ses.pd(calli_matrix, calli_tree, include.root = TRUE, null.model = "taxa.label")
...
#Saving results
#write.csv(file = "results/Calliandra/pd_stats.csv", pd_stats, row.names = FALSE)

#Loading results
#pd_stats <- read.csv("results/Calliandra/pd_stats.csv")
...
pd_plot <- spplot(pd_poly,
                  ...
                  colorkey = list(labels = list(at = seq(0, 0.4, length.out = 5))), 
                  at = seq(0, 0.471, length.out = 16),
...
corPdsr_plot <- ggplot(data = pd_stats, mapping = aes(jitter(ntaxa), pd.obs))+
  geom_jitter()+
  labs(title = "Calliandra", subtitle = paste("r =", corPdsr))+
  xlab("Richness")+
...
pdses_plot <- spplot(pd_poly,
                     ...
                     xlim = c(-50.5, -38.5), ylim = c(-23.25, -8.5),
                     par.settings=list(fontsize = list(text = 21)),
                     at = seq(-2.611, 2.611, length.out = 16),
                     colorkey = TRUE, 
                     sp.layout = list(list(br, fill = "gray")), 
...
pdres_plot <- spplot(pd_poly,
                     ...
                     xlim = c(-50.5, -38.5), ylim = c(-23.25, -8.5),
                     par.settings=list(fontsize = list(text = 21)),
                     at = seq(-0.081, 0.081, length.out = 16),
                     colorkey = TRUE, 
                     sp.layout = list(list(br, fill = "gray")),

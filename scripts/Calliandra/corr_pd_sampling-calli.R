...
coords <- read.csv(file = "datasets/Calliandra/calli_cr.csv", na.strings = c("", NA))

# Reading PD results
pd <- read.csv("results/Calliandra/pd_stats.csv")
pd <- pd %>% rename("id_grid" = id)

# Reading tree
tree <- read.nexus("trees/Calliandra/pruned_tree-calli.nex")
...
corrPdRed_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Calliandra", subtitle = paste("r =", corrPdRed, "\np-value < 0.05, R² = 0.16 "))+
  xlab("Redundancy")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text(face = "italic"))+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Calliandra/calli-corrPdRed_plot.pdf"); corrPdRed_plot; dev.off()
...
corrPdN_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Calliandra", subtitle = paste("r =", corrPdN, "\np < 0.05, R² = 0.42"))+
  xlab("Number of samples")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text(face = "italic"))+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Calliandra/calli-corrPdN_plot.pdf"); corrPdN_plot; dev.off()

...
jaccard_matrix <- as.matrix(vegdist(calli_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Calliandra/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Calliandra/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Calliandra/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(calli_matrix, dist = "jaccard",
...
png("plots/Calliandra/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Calliandra/upgma/jaccard_dend.pdf", 

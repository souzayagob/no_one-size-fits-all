...
#Calculating PE
pe <- data.frame("id" = rownames(calli_matrix), 
                 "pe" = phyloendemism(calli_matrix, calli_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(calli_tree, calli_matrix))
rownames(null.output) <- rownames(calli_matrix)
...
#Tree for comparison
comp_tree <- calli_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(calli_matrix), 
                      "pe_comp" = phyloendemism(calli_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, calli_matrix))
rownames(null.output) <- rownames(calli_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(calli_tree, calli_matrix))
rownames(null.output) <- rownames(calli_matrix)
...
png("plots/Calliandra/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Calliandra/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Calliandra/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Calliandra/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Calliandra/canape/canape_plot.png",

...
#jaccard_matrix <- as.matrix(read.csv("results/Mimosa/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Mimosa/unifrac_matrix.csv", row.names = 1))
...
pdf("plots/Mimosa/upgma/jaccard_plot-6.pdf",
    height = 4, width = 4); jaccard_plot; dev.off()
pdf("plots/Mimosa/upgma/jaccard_dend-6.pdf", 

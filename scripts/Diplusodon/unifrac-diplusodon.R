...
diplusodon_tree <- drop.tip(diplusodon_tree, 
                        diplusodon_tree$tip.label[!diplusodon_tree$tip.label %in% colnames(diplusodon_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(diplusodon_matrix, diplusodon_tree))
write.csv(file = "results/Diplusodon/unifrac_matrix.csv", unifrac_matrix)

#Correlation between distance and UniFrac
dist_grids <- as.matrix(read.csv("results/dist_grids.csv", row.names = 1))
colnames(dist_grids) <- rownames(dist_grids)
dist_grids <- dist_grids[rownames(unifrac_matrix), colnames(unifrac_matrix)]
lm(dist_grids ~ unifrac_matrix)
cor(c(dist_grids), c(unifrac_matrix))

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(diplusodon_matrix, diplusodon_tree, dist = "unifrac",
...
png("plots/Diplusodon/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Diplusodon/upgma/unifrac_dend.pdf", 

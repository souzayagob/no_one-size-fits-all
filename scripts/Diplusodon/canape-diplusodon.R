...
#Reading tree
diplusodon_tree <- read.nexus("trees/Diplusodon/pruned_tree-diplusodon.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(diplusodon_matrix), 
                 "pe" = phyloendemism(diplusodon_matrix, diplusodon_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(diplusodon_tree, diplusodon_matrix))
rownames(null.output) <- rownames(diplusodon_matrix)
...
#Tree for comparison
comp_tree <- diplusodon_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(diplusodon_matrix), 
                      "pe_comp" = phyloendemism(diplusodon_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, diplusodon_matrix))
rownames(null.output) <- rownames(diplusodon_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(diplusodon_tree, diplusodon_matrix))
rownames(null.output) <- rownames(diplusodon_matrix)
...
png("plots/Diplusodon/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Diplusodon/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Diplusodon/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Diplusodon/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Diplusodon/canape/canape_plot.png",

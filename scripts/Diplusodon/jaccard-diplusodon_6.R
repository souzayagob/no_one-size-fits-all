...
coman_matrix <- coman_matrix[ , which(colSums(coman_matrix) > 1)]

#Removing empty sites 
coman_matrix <- coman_matrix[which(rowSums(coman_matrix) > 0), ]
...
jaccard_matrix <- as.matrix(vegdist(coman_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Comanthera/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Comanthera/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Comanthera/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(coman_matrix, dist = "jaccard",
...
pdf("plots/Comanthera/upgma/jaccard_plot-6.pdf",
    height = 4, width = 4); jaccard_plot; dev.off()
pdf("plots/Comanthera/upgma/jaccard_dend-6.pdf", 
    width = 4, height = 4); plot_horiz.dendrogram(dend, axes = F); dev.off()

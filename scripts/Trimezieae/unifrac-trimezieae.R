...
trimezieae_matrix <- trimezieae_matrix[ , which(colSums(trimezieae_matrix) > 1)]

#Removing empty sites 
trimezieae_matrix <- trimezieae_matrix[which(rowSums(trimezieae_matrix) > 0), ]
...
trimezieae_tree <- drop.tip(trimezieae_tree, 
                           trimezieae_tree$tip.label[!trimezieae_tree$tip.label %in% colnames(trimezieae_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(trimezieae_matrix, trimezieae_tree))
write.csv(file = "results/Trimezieae/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(trimezieae_matrix, trimezieae_tree, dist = "unifrac",
...
png("plots/Trimezieae/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Trimezieae/upgma/unifrac_dend.pdf", 

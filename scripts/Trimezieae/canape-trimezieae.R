...
#Reading matrix
trimezieae_matrix <- read.csv(file = "matrices/trimezieae_matrix.csv", row.names = 1)

#Reading tree
trimezieae_tree <- read.nexus("trees/Trimezieae/pruned_tree-trimezieae.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(trimezieae_matrix), 
                 "pe" = phyloendemism(trimezieae_matrix, trimezieae_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(trimezieae_tree, trimezieae_matrix))
rownames(null.output) <- rownames(trimezieae_matrix)
...
#Tree for comparison
comp_tree <- trimezieae_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(trimezieae_matrix), 
                      "pe_comp" = phyloendemism(trimezieae_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, trimezieae_matrix))
rownames(null.output) <- rownames(trimezieae_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(trimezieae_tree, trimezieae_matrix))
rownames(null.output) <- rownames(trimezieae_matrix)
...
png("plots/Trimezieae/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Trimezieae/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Trimezieae/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Trimezieae/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Trimezieae/canape/canape_plot.png",

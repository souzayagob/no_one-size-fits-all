...
jaccard_matrix <- as.matrix(vegdist(trimezieae_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Trimezieae/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Trimezieae/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Trimezieae/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(trimezieae_matrix, dist = "jaccard",
...
png("plots/Trimezieae/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Trimezieae/upgma/jaccard_dend.pdf", 

...
coords <- read.csv(file = "datasets/Marcetieae/marcetieae_cr.csv", na.strings = c("", NA))

# Reading PD results
pd <- read.csv("results/Marcetieae/pd_stats.csv")
pd <- pd %>% rename("id_grid" = id)

# Reading tree
tree <- read.nexus("trees/Marcetieae/pruned_tree-marcetieae.nex")
...
corrPdRed_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Marcetieae", subtitle = paste("r =", corrPdRed, "\np-value < 0.05, R² = 0.24"))+
  xlab("Redundancy")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text())+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Marcetieae/marcetieae-corrPdRed_plot.pdf"); corrPdRed_plot; dev.off()
...
corrPdN_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Marcetieae", subtitle = paste("r =", corrPdN, "\np < 0.05, R² = 0.44"))+
  xlab("Number of samples")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text())+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Marcetieae/marcetieae-corrPdN_plot.pdf"); corrPdN_plot; dev.off()

...
marcetieae_matrix <- marcetieae_matrix[ , which(colSums(marcetieae_matrix) > 1)]

#Removing empty sites 
marcetieae_matrix <- marcetieae_matrix[which(rowSums(marcetieae_matrix) > 0), ]
...
marcetieae_tree <- drop.tip(marcetieae_tree, 
                            marcetieae_tree$tip.label[!marcetieae_tree$tip.label %in% colnames(marcetieae_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(marcetieae_matrix, marcetieae_tree))
write.csv(file = "results/Marcetieae/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(marcetieae_matrix, marcetieae_tree, dist = "unifrac",
...
png("plots/Marcetieae/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Marcetieae/upgma/unifrac_dend.pdf", 

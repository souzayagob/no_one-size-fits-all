...
set.seed(7)
pd_stats <- ses.pd(marcetieae_matrix, marcetieae_tree, include.root = TRUE, null.model = "taxa.label")
...
#Saving results
#write.csv(file = "results/Marcetieae/pd_stats.csv", pd_stats, row.names = FALSE)

#Loading results
#pd_stats <- read.csv("results/Marcetieae/pd_stats.csv")
...
corPdsr_plot <- ggplot(data = pd_stats, mapping = aes(jitter(ntaxa), pd.obs))+
  geom_jitter()+
  labs(title = "Marcetieae", subtitle = paste("r =", corPdsr))+
  xlab("Richness")+
...
pdses_plot <- spplot(pd_poly,
                     ...
                     xlim = c(-50.5, -38.5), ylim = c(-23.25, -8.5),
                     par.settings=list(fontsize = list(text = 21)),
                     at = seq(-2.961, 2.961, length.out = 16),
                     colorkey = TRUE, 
                     sp.layout = list(list(br, fill = "gray")), 
...
pdres_plot <- spplot(pd_poly,
                     ...
                     xlim = c(-50.5, -38.5), ylim = c(-23.25, -8.5),
                     par.settings=list(fontsize = list(text = 21)),
                     at = seq(-0.191, 0.191, length.out = 16),
                     colorkey = TRUE, 
                     sp.layout = list(list(br, fill = "gray")), 

...
marcetieae_clean <- read.csv(file = "datasets/Marcetieae/marcetieae_clean.csv", na.strings = c("", NA))
...
#Saving results
write.csv(file = "results/Marcetieae/redundancy.csv", redundancy, row.names = FALSE)
...
#Loading results
redundancy <- read.csv("results/Marcetieae/redundancy.csv")
...
redMean_plot <- ggplot(data = redundancy, mapping = aes(x = grid_size, y = mean)) + 
  ...
  theme_bw()+
png("plots/Marcetieae/redundancy/redMean_plot.png",

...
#Reading matrix
marcetieae_matrix <- read.csv(file = "matrices/marcetieae_matrix.csv", row.names = 1)

#Reading tree
marcetieae_tree <- read.nexus("trees/Marcetieae/pruned_tree-marcetieae.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(marcetieae_matrix), 
                 "pe" = phyloendemism(marcetieae_matrix, marcetieae_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(marcetieae_tree, marcetieae_matrix))
rownames(null.output) <- rownames(marcetieae_matrix)
...
#Tree for comparison
comp_tree <- marcetieae_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(marcetieae_matrix), 
                      "pe_comp" = phyloendemism(marcetieae_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, marcetieae_matrix))
rownames(null.output) <- rownames(marcetieae_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(marcetieae_tree, marcetieae_matrix))
rownames(null.output) <- rownames(marcetieae_matrix)
...
png("plots/Marcetieae/canape/pe_plot.png",
    height = 4, width = 4, units = 'in', res=300); pe_plot; dev.off()
png("plots/Marcetieae/canape/peP_plot.png",
    height = 4, width = 4, units = 'in', res=300); peP_plot; dev.off()
png("plots/Marcetieae/canape/rpe_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpe_plot; dev.off()
png("plots/Marcetieae/canape/rpeP_plot.png",
    height = 4, width = 4, units = 'in', res=300); rpeP_plot; dev.off()
png("plots/Marcetieae/canape/canape_plot.png",

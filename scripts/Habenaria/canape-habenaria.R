...
#Reading matrix
habenaria_matrix <- read.csv(file = "matrices/habenaria_matrix.csv", row.names = 1)

#Reading tree
habenaria_tree <- read.nexus("trees/Habenaria/pruned_tree-habenaria.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(habenaria_matrix), 
                 "pe" = phyloendemism(habenaria_matrix, habenaria_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(habenaria_tree, habenaria_matrix))
rownames(null.output) <- rownames(habenaria_matrix)
...
#Tree for comparison
comp_tree <- habenaria_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(habenaria_matrix), 
                      "pe_comp" = phyloendemism(habenaria_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, habenaria_matrix))
rownames(null.output) <- rownames(habenaria_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(habenaria_tree, habenaria_matrix))
rownames(null.output) <- rownames(habenaria_matrix)

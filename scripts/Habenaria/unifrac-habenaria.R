...
habenaria_matrix <- habenaria_matrix[ , which(colSums(habenaria_matrix) > 1)]

#Removing empty sites 
habenaria_matrix <- habenaria_matrix[which(rowSums(habenaria_matrix) > 0), ]
...
habenaria_tree <- drop.tip(habenaria_tree, 
                      habenaria_tree$tip.label[!habenaria_tree$tip.label %in% colnames(habenaria_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(habenaria_matrix, habenaria_tree))
write.csv(file = "results/Habenaria/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(habenaria_matrix, habenaria_tree, dist = "unifrac",
...
png("plots/Habenaria/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Habenaria/upgma/unifrac_dend.pdf", 

...
minaria_matrix <- minaria_matrix[ , which(colSums(minaria_matrix) > 1)]

#Removing empty sites 
minaria_matrix <- minaria_matrix[which(rowSums(minaria_matrix) > 0), ]
...
minaria_tree <- drop.tip(minaria_tree, 
                       minaria_tree$tip.label[!minaria_tree$tip.label %in% colnames(minaria_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(minaria_matrix, minaria_tree))
write.csv(file = "results/Minaria/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(minaria_matrix, minaria_tree, dist = "unifrac",
...
png("plots/Minaria/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Minaria/upgma/unifrac_dend.pdf", 

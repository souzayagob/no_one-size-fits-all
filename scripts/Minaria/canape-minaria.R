...
#Reading matrix
minaria_matrix <- read.csv(file = "matrices/minaria_matrix.csv", row.names = 1)

#Reading tree
minaria_tree <- read.nexus("trees/Minaria/pruned_tree-minaria.nex")
...
#Calculating PE
pe <- data.frame("id" = rownames(minaria_matrix), 
                 "pe" = phyloendemism(minaria_matrix, minaria_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(minaria_tree, minaria_matrix))
rownames(null.output) <- rownames(minaria_matrix)
...
#Tree for comparison
comp_tree <- minaria_tree
comp_tree$edge.length <- rep(1/length(comp_tree$edge.length), 
                             length(comp_tree$edge.length)) #equal branch lengths

#Calculating PE
pe_comp <- data.frame("id" = rownames(minaria_matrix), 
                      "pe_comp" = phyloendemism(minaria_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, minaria_matrix))
rownames(null.output) <- rownames(minaria_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(minaria_tree, minaria_matrix))
rownames(null.output) <- rownames(minaria_matrix)

...
cairo_pdf("figures/Minaria/minaria-corrPdRed_plot.pdf"); corrPdRed_plot; dev.off()
...
corrPdN_plot <- ggplot(data = pd_red, mapping = aes(jitter(redundancy), pd.obs))+
  geom_jitter()+
  labs(title = "Minaria", subtitle = paste("r =", corrPdN, "\np < 0.05, R² = 0.40"))+
  xlab("Number of samples")+
  ylab("Phylogenetic diversity")+
  theme_bw(base_size = 14)+
  theme(plot.title = element_text(face = "italic"))+
  geom_smooth(method='lm', formula= y~x)
cairo_pdf("figures/Minaria/minaria-corrPdN_plot.pdf"); corrPdN_plot; dev.off()

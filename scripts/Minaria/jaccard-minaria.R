...
jaccard_matrix <- as.matrix(vegdist(minaria_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Minaria/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Minaria/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Minaria/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(minaria_matrix, dist = "jaccard",
...
png("plots/Minaria/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Minaria/upgma/jaccard_dend.pdf", 

...
coman_tree <- drop.tip(coman_tree, 
                        coman_tree$tip.label[!coman_tree$tip.label %in% colnames(coman_matrix)])
...
#UniFrac matrix
unifrac_matrix <- as.matrix(unifrac(coman_matrix, coman_tree))
write.csv(file = "results/Comanthera/unifrac_matrix.csv", unifrac_matrix)

#===============#
# Running UPGMA #
#===============#

upgma <- recluster.cons(coman_matrix, coman_tree, dist = "unifrac",
...
png("plots/Comanthera/upgma/unifrac_plot.png",
    height = 4, width = 4, units = 'in', res=300); unifrac_plot; dev.off()
cairo_pdf("plots/Comanthera/upgma/unifrac_dend.pdf", 

...
#Calculating PE
pe <- data.frame("id" = rownames(catt_matrix), 
                 "pe" = phyloendemism(catt_matrix, catt_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(catt_tree, catt_matrix))
rownames(null.output) <- rownames(catt_matrix)
...
#Calculating PE
pe_comp <- data.frame("id" = rownames(catt_matrix), 
                      "pe_comp" = phyloendemism(catt_matrix, comp_tree, weighted = T)) 
...
set.seed(7)
null.output <- replicate(999, rand.matrix.pe(comp_tree, catt_matrix))
rownames(null.output) <- rownames(catt_matrix)
...
set.seed(7)
null.output <- replicate(999, rand.matrix.rpe(catt_tree, catt_matrix))
rownames(null.output) <- rownames(catt_matrix)

...
jaccard_matrix <- as.matrix(vegdist(catt_matrix, method = "jaccard", diag = TRUE))
write.csv(file = "results/Cattleya/jaccard_matrix.csv", jaccard_matrix)

#CODE FOR CORRELATION
#jaccard_matrix <- as.matrix(read.csv("results/Cattleya/jaccard_matrix.csv", row.names = 1))
#colnames(jaccard_matrix) <- gsub("X", "", colnames(jaccard_matrix))
#unfirac_matrix <- as.matrix(read.csv("results/Cattleya/unifrac_matrix.csv", row.names = 1))
...
upgma <- recluster.cons(catt_matrix, dist = "jaccard",
...
png("plots/Cattleya/upgma/jaccard_plot.png",
    height = 4, width = 4, units = 'in', res=300); jaccard_plot; dev.off()
cairo_pdf("plots/Cattleya/upgma/jaccard_dend.pdf", 
